...
#
# Needle: QUEUE_KNOWN_KEYS_CONTENT_FP_2026_09_07 — WQ.touch() remiss flips mtime
# so generation MISS re-scanned ~2ms though content unchanged (gather remiss /
# compact theater). Content fp (hash of loaded md pair) HIT skips parse; mtime
# stamp still refreshed so subsequent disk-default probes stay sub-ms.
_QUEUE_KNOWN_CACHE: dict[str, Any] = {"key": None, "keys": None, "content_fp": None}
...
def _queue_known_content_fp(work_md: str, context_md: str) -> tuple[int, int]:
    """Stable content token for loaded WQ+context blobs (survives mtime-only touch)."""
...
def queue_known_keys(*, work_md: str | None = None, context_md: str | None = None) -> set[str]:
    ...
    ``rank_opportunities`` cache probes stay sub-ms. After mtime remiss, content
    fp HIT skips the open+done scan when blobs are unchanged. Needle:
    QUEUE_KNOWN_KEYS_GENERATION_2026_09_07 + QUEUE_KNOWN_KEYS_CONTENT_FP_2026_09_07.
    """
    ...
    if (
        _QUEUE_KNOWN_CACHE.get("content_fp") == content_fp
        and _QUEUE_KNOWN_CACHE.get("keys") is not None
    ):
        keys = set(_QUEUE_KNOWN_CACHE["keys"])  # type: ignore[arg-type]
        if use_disk_cache:
    ...
    _QUEUE_KNOWN_CACHE["content_fp"] = content_fp
...
# adapt.run_audit (~8–9ms, git shell) while kit/git generation unchanged. Queue
# drift stays on sync_queue_drift; audit TTL mirrors stall STALL_ADAPT_AUDIT_TTL.
# Hub port L63 after gather generation-skip (L62).
...
def _gather_audit_witness() -> tuple[Any, ...]:
    ...
    index every wake and forced remiss (~8ms run_audit) despite unchanged HEAD/kit.
    """
    try:
...
# Needle: GATHER_SIGNALS_INPUT_TTL_2026_09_05 / GATHER_SIGNALS_GENERATION_ONLY_2026_09_05
# Hub port L62 — improve-loop paid full gather every tick (~17–28ms) after rank HIT.
# Generation key (pair mtime_ns + flags); wall age ignored (ttl<=0 forces miss).
# Bypass when live_state injected or refresh_trends forces fresh research.
_GATHER_CACHE: dict[str, Any] = {"key": None, "at": 0.0, "signals": None}


def gather_signals_ttl_sec() -> float:
    """Legacy CFG knob — ``<=0`` forces miss; positive values ignored (generation-only).

    Needle: GATHER_SIGNALS_GENERATION_ONLY_2026_09_05 — wall TTL remissed same
    pair+flags after prepare/write_prompts already generation-skip.
    """
    try:
        raw = auto.CFG.get("gather_signals_ttl_sec")
        if raw is None:
            return 120.0
...
def clear_gather_signals_cache() -> None:
    """Drop gather_signals generation cache (tests + forced refresh)."""
    _GATHER_CACHE["key"] = None
    _GATHER_CACHE["at"] = 0.0
    _GATHER_CACHE["signals"] = None
    clear_gather_audit_cache()
    clear_rank_opportunities_cache()


def _queue_pair_mtime_ns() -> tuple[int, int]:
    """WORK_QUEUE + self_improve_context mtime_ns — gather input generation."""
    wq = auto.WORK_QUEUE_PATH
    ctx = auto.CONTEXT_PATH
    w_ns = int(wq.stat().st_mtime_ns) if wq.is_file() else 0
    c_ns = int(ctx.stat().st_mtime_ns) if ctx.is_file() else 0
    return w_ns, c_ns


def _gather_input_key(
    *,
    quick: bool,
    research: bool,
    digest_only: bool,
    ...
    p = _queue_pair_mtime_ns() if pair is None else pair
    return (p[0], p[1], bool(quick), bool(research), bool(digest_only), bool(audit))


def _gather_cache_get(key: tuple[Any, ...], *, now: float | None = None) -> ImproveSignals | None:
    """Return cached signals when key matches (generation-only; wall age ignored).

    ``gather_signals_ttl_sec() <= 0`` forces miss. ``now`` kept for API compat.
    """
    del now  # generation-only; wall age intentionally ignored
    ttl = gather_signals_ttl_sec()
    if ttl <= 0:
        return None
    if _GATHER_CACHE.get("key") != key:
        return None
    cached = _GATHER_CACHE.get("signals")
    if cached is None:
        return None
    return cached  # type: ignore[return-value]


def _gather_cache_put(key: tuple[Any, ...], signals: ImproveSignals, *, now: float | None = None) -> None:
    _GATHER_CACHE["key"] = key
    _GATHER_CACHE["at"] = time.monotonic() if now is None else now
    _GATHER_CACHE["signals"] = signals


# Needle: DUAL_GATHER_COOLDOWN_DISK_BRIEF_2026_09_07 — audit gather remiss imported
# peer_dual_research every tick (~4ms) solely to read RESEARCH_SYNC; cooldown=0
# paid run_research_cycle (~280ms) on the improve hot path. Disk brief + state
...
def _resolve_gather_research_sync(
    *,
    ...
    - DUAL_RESEARCH_COOLDOWN_GATHER_REMISS_2026_09_07 — ballast-drop remiss must not
      cold-import dual solely for cooldown+brief (~8–27ms).
    """
    ...
    try:
        if (not audit_cache_hit) and _dual_research_cooldown_remaining() <= 0:
            ...
            # force=False → DUAL_RESEARCH_WITNESS_SKIP can skip probe+write when
            # WQ/loop inputs unchanged after cooldown (gather remiss residual).
...
def gather_signals(
    *,
    ...
    use_cache = live_state is None and not refresh_trends
    cache_key: tuple[Any, ...] | None = None
    if use_cache:
        cache_key = _gather_input_key(
            quick=quick, research=research, digest_only=digest_only, audit=audit
        )
        hit = _gather_cache_get(cache_key)
        if hit is not None:
            return hit
    ...
    if digest_only or not audit:
        ...
        # GATHER_ADAPT_AUDIT_GEN_TTL_2026_09_07 — skip run_audit on WQ remiss
    ...
    if use_cache and cache_key is not None:
        _gather_cache_put(cache_key, signals)
    return signals
...
def _rank_input_key(
    ...
    *,
    ...
    try:
        ...
        team_key = team._team_status_input_key(quick=True)
    except Exception:  # noqa: BLE001
        team_key = ()
