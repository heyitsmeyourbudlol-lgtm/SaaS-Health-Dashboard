...
def _parse_phased_work_items(work_md: str) -> list[str]:
    ...
    if not work_md:
        _parse_phased_result = None
        return []
    # Identity short-circuit — same str object within TTL skips sha256.
    md_id = id(work_md)
    now = time.time()
    if (
        _parse_phased_result is not None
        and _parse_phased_md_id == md_id
        and (now - _parse_phased_at) < PARSE_PHASED_CACHE_TTL_SEC
    ):
        return list(_parse_phased_result)
    # zlib — avoid hashlib→libcrypto (~4MB) on every cold import of this module.
    import zlib

    raw = work_md.encode("utf-8", errors="replace")
    fp = f"{zlib.adler32(raw) & 0xffffffff:08x}{zlib.crc32(raw) & 0xffffffff:08x}"
    if (
        _parse_phased_result is not None
        and _parse_phased_fp == fp
        and (now - _parse_phased_at) < PARSE_PHASED_CACHE_TTL_SEC
    ):
        return list(_parse_phased_result)
    ...
    out: list[str] = []
    in_track = False
    # OVERSEER_PHASED_STOP_REMAINING_PRODUCT_2026_09_07 — Active must end before
    # Remaining / Product rules; else plain ``- sync`` under Product rules is
    # counted as Active open → dual-brain + heal removes Product chrome.
    stop_prefixes = (
        "## Creative backlog",
        "## Done",
        "## Metrics",
    ...
    for line in lines:
        ...
        if in_track:
            ...
            if item:
                out.append(item)
    _parse_phased_fp = fp
    _parse_phased_md_id = md_id
    _parse_phased_at = now
    _parse_phased_result = out
    return list(out)
...
def _kit_run_receipt_landed(item: str) -> bool:
    ...
    if not word:
        return False
    receipt = ROOT / "notes" / f"KIT_A_TO_Z_{word.upper()}_TARGET.md"
    try:
...
def _kit_run_receipt_landed_line(item: str) -> str | None:
    ...
    bits = [
        ...
        f"receipt `notes/KIT_A_TO_Z_{word.upper()}_TARGET.md`",
    ]
    if wt:
        bits.append(f"wt=`{wt}`")
    if branch:
...
def _insert_creative_open_lines(md: str, demoted_lines: list[str]) -> str:
    ...
    for dl in demoted_lines:
        ...
        if key in existing:
            continue
        to_add.append(_annotate_human_auth_demote(dl.strip()))
        existing.add(key)
    if not to_add:
        return md
    out: list[str] = []
...
def demote_human_auth_blocked_active(
    ...
    def _scan_active(md: str) -> tuple[str, list[str]]:
        ...
        for line in lines:
            ...
            if in_active and s.startswith("- [ ]"):
                item = _parse_work_item(s)
                if item and is_human_auth_blocked_item(item):
                    moved.append(line)
    ...
    if not demoted_keys and not demoted_lines:
        ...
        ctx2, rem_n = _strip_open_keys_from_remaining(
            ctx_md,
            {
                _normalize_queue_key(i)
                for i in remaining_work_items(ctx_md)
                if is_human_auth_blocked_item(i)
            },
        )
        if rem_n:
            return work_md, ctx2, rem_n
    ...
    ctx_new, rem_n = _strip_open_keys_from_remaining(ctx_new, demoted_keys)
    # Also strip any other human-auth Remaining orphans.
    rem_keys = {
        _normalize_queue_key(i)
        for i in remaining_work_items(ctx_new)
        if is_human_auth_blocked_item(i)
    }
    if rem_keys:
        ctx_new, rem_n2 = _strip_open_keys_from_remaining(ctx_new, rem_keys)
        rem_n += rem_n2
