...
class TestPeerCommands(unittest.TestCase):
    ...
    def test_error_adapt_registered(self) -> None:
        # OVERSEER_ERROR_ADAPT_PEER_WRAP_2026_09_08 — log language alias
        ids = {c.id for c in pc.COMMANDS}
        self.assertIn("error-adapt", ids)
        cmd = next(c for c in pc.COMMANDS if c.id == "error-adapt")
        self.assertTrue(cmd.pivotal)
        ...
        self.assertIn("error-adapt)", peer_sh)
        report = pc.run_command("error-adapt", dry_run=True)
        self.assertTrue(report.ok)
