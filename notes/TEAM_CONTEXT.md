...
_Updated 2026-09-08 00:03:41_ · read before Plan/Implement — includes low-importance facts that can change output
...
- `kit_a_to_z`: 2026-09-07: peer-cycle f281724 clobbered heal_false_green_lock from factory_kit_run.py while PROOF stayed green; restore from pre-clobber + fix _commit_receipt 
- `kit_a_to_z`: output-compare: match — kit_a_to_z false-green heal + live kit-run
- `communications_engineer`: 2026-09-07: COMMS_HORIZON Self-test RED was test_vault_prompt_omits_state_path — schema.caps lacked vault_omit_state_path and format_vault_block still emitted -
- `communications_engineer`: 2026-09-07 CE: MCP REQ schema already hub-landed (mcp:<tool>); serialize ASN was NOTES ONLY — flip automation_comms_research kit_status partial→have + horizon R
- `communications_engineer`: output-compare: match — ce:vault_omit
- `communications_engineer`: output-compare: match — ce-mcp-doc HAVE+horizon
