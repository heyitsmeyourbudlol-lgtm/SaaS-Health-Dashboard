...
def non_noop_day_stats(
    ...
    *,
    ...
    meets = today_n >= bar or (hours_elapsed >= 4.0 and projected >= bar)
    return {
        "today": today,
...
def find_latest_transcript(*, fresh: bool = False) -> Path | None:
    ...
    if (
        not fresh
        and _LATEST_TRANSCRIPT_CACHE is not None
        and _LATEST_TRANSCRIPT_CACHE[1] == fp
        and now - _LATEST_TRANSCRIPT_CACHE[0] < _LATEST_TRANSCRIPT_TTL_SEC
    ):
...
def queue_fingerprint(items: list[str]) -> str:
    ...
    """
    keys = sorted(auto._normalize_queue_key(i) for i in items if i.strip())
    raw = "\n".join(keys).encode()
...
class PeerEvent:
    ...
    reason: str  # transcript | signal | git | timeout | fallback
...
class PeerEventWatcher:
    ...
    def watch_path(self, path: Path) -> bool:
        """Add an extra VNODE watch (logs, signals). Call after setup()."""
    ...
    def wait(self, *, timeout: float | None = None) -> PeerEvent:
        if not self._available or self._kq is None:
            # No kqueue — wall-clock sleep is the only option on Linux/CI.
            time.sleep(timeout if timeout is not None else FALLBACK_POLL_SEC)
