...
class ClearDeferredStampAfterReadyTests(unittest.TestCase):
    ...
    def test_ready_clears_deferred_last_cycle_stamp(self) -> None:
        ...
        with unittest.mock.patch.object(pt, "load_state", return_value=state), unittest.mock.patch.object(
            pt, "current_queue_fingerprint", return_value=("fp", [])
        ), unittest.mock.patch.object(pt, "save_state") as save, unittest.mock.patch.object(
            pt, "git_head_oneline", return_value="deadbeef oneline"
        ), unittest.mock.patch.object(pt, "scrub_last_cycle_poison", side_effect=lambda s: s):
